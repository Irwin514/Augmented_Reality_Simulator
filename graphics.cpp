...
#define TB_SIZE BUFFER_SIZE1

// maximum buffer size for drawing line and point lists
// BUFFER_SIZE3 = BUFFER_SIZE1 / 4 = 15750 -> 7875 lines
#define LB_SIZE BUFFER_SIZE3

// arrays for storing DirectX vertices
static double XB[TB_SIZE],YB[TB_SIZE],ZB[TB_SIZE];
static double RB[TB_SIZE],GB[TB_SIZE],BB[TB_SIZE];
static double UB[TB_SIZE],VB[TB_SIZE];
...
// assume curve is closed -> last point n-1 is joined to first point 0
...
// 3 - Y-Z (i-j) coordinates
{
	...
	if( ni*nj > k ) {
		q = (double) k / (ni*nj);
		nj *= q; // shrink the rectangular domain
	}
